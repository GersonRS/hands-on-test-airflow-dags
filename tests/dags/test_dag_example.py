...
@pytest.mark.parametrize("dag_id", get_dags())
def test_task_requirements(dag_id, dag):
    assert dag.task, f"DAG {dag_id} has no tasks."
    for task in dag.tasks:
        assert (
            task.trigger_rule == "all_success"
        ), f"Task {task.task_id} in DAG {dag_id} does not use 'all_success'."
