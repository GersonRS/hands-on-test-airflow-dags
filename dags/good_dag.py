...
BASE_DATA_PATH = os.getenv("BASE_DATA_PATH", os.path.dirname(os.path.abspath(__file__)) + "/data/")
IMPORT_PATH_CUSTOMER_FEEDBACK = f"{BASE_DATA_PATH}customer_feedback3.csv"
IMPORT_CUSTOMER_DATA = f"{BASE_DATA_PATH}customer_data3.csv"
IMPORT_PATH_SALES_REPORTS = f"{BASE_DATA_PATH}sales_reports3.csv"
...
def good_dag():
    ...
    def analyze_sentiment(comment: str) -> float:
        ...
        from src.hugging_face_functions import get_sentiment_score
    ...
    def calculate_purchases_per_toy(df: pd.DataFrame, **context) -> pd.DataFrame:
        ...
        task_logger.warn(f"The average number of purchases per toy is {df['QuantitySold'].mean()}")
